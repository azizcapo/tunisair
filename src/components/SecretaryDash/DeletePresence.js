...
const DeletePresence = ({presence,reload,setReload,open,handleClose}) => {
    ...
    const notifyError = () => {
      ...
      const handleDelete = async () => {
        try {
          ...
          if (token) {
                await axios.delete(
                    `http://localhost:3010/api/invitation/remove/${.id}`
                  );
                  notify();
                  handleClose();
                  setReload(!reload)
      ...
      <Fade in={open}>
        <Box sx={style}>
          <div className="filiale-form-delete">
            <p className="delete">Vous êtes sure vous voulez supprimer ce Document ?</p>
