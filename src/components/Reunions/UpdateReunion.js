...
const UpdateReunion = ({reload,setReload,reunion,open,handleClose}) => {
  const [options,setOptions] = useState([])
  const [type, setType] = useState(reunion.type)
  const [users,setUsers] = useState([]);
  const [selectedUsers,setSelectedUsers] = useState([])
  const [name, setName] = useState(reunion.name)
  const [date, setDate] = useState(reunion.date.substring(0,10))
      ...
      if (token) {
        if(name && date && type ){
          const res = await axios.put(
            `http://localhost:3010/api/reunion/update/${reunion.id}`,
            body
          );
          notify();
          handleClose();
